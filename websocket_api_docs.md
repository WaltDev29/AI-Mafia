...
- **URL:** `ws://<HOST>/ws?mode=<solo|group>&nickname=<문자열>`
- **파라미터:**
  - `mode`: `solo` (인간 1 vs AI 4) 또는 `group` (인간 4 vs AI 1)
  - `nickname`: 사용자 닉네임 (1~20자)
...
sequenceDiagram
    ...
    %% 연결 및 대기
    C->>S: WebSocket Connect (mode, nickname)
...
| `game_start` | `{ "game_id": "...", "mode": "solo", "players": [ { "id": "...", "nickname": "...", "is_human": true, "is_eliminated": false } ] }` | 게임 시작 및 참가자 전체 정보 |
| `game_over` | `{ "result": "human_win"\|"ai_win", "message": "...", "players": [...] }` | 게임 종료 및 최종 승패 |
| `error` | `{ "detail": "에러 메시지" }` | 규칙 위반 또는 서버 에러 발생 시 |
| `player_eliminated`| `{ "player_id": "...", "nickname": "...", "reason": "연결 끊김" }` | 접속 종료 등으로 인한 비정상 탈락 발생 시 |

### 4.2. 라운드 및 페이즈 이벤트
| 이벤트(type) | 데이터(data) 페이로드 예시 | 설명 |
|---|---|---|
| `round_start` | `{ "round": 1, "prompt_word": "첫사랑", "order": [{"id": "...", "nickname": "..."}] }` | 라운드 시작 및 이번 라운드 제시어와 발언 순서 |
| `experience_request`| `{ "current_player_id": "...", "current_nickname": "...", "timeout": 30 }` | **(중요)** 현재 경험담을 제출해야 하는 타자 정보 |
| `experience_submitted`| `{ "player_id": "...", "nickname": "...", "content": "내용" }` | 누군가 경험담을 성공적으로 제출함 |
| `experience_timeout`| `{ "player_id": "...", "nickname": "..." }` | 타임아웃으로 제출을 패스함 |
| `free_chat_start` | `{ "duration": 120 }` | 자유 대화 시작. 이 때부터 채팅 전송 가능 |
| `free_chat_end` | `{}` (빈 객체) | 자유 대화 종료 알림 |
| `chat_message` | `{ "player_id": "...", "nickname": "...", "content": "내용", "timestamp": 161... }` | 누군가 채팅을 보냄 (브로드캐스트) |
| `typing_status` | `{ "player_id": "...", "nickname": "...", "is_typing": true }` | 누군가 타이핑을 치고 있음 |

### 4.3. 판정 이벤트
| 이벤트(type) | 데이터(data) 페이로드 예시 | 설명 |
|---|---|---|
| `judging_start` | `{}` (빈 객체) | LLM이 대화를 분석하기 시작함 |
| `judge_result` | `{ "eliminated_player_id": "...", "eliminated_nickname": "...", "human_probability": 85, "reason": "...", "was_human": true\|false }` | LLM 판정 결과 및 해당 라운드 탈락자 공개 |
